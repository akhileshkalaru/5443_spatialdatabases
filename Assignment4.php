...
$db = new mysqli('localhost', 'akalaru', 'akhima97971', 'akalaru');
	  ...
	  var map = new google.maps.Map(document.getElementById('map-canvas'),
		...
		if(sizeof($_POST)>0){
			...
			while($line = $result->fetch_assoc()){
				...
				echo"position: new google.maps.LatLng({$line['latitude']},{$line['longitude']}),\n";
				echo"map: map,\n";
				echo"title:\"{$line['fullname']}\"\n";
				echo"});\n";
				$i++;
				if($j<$_POST['base']){
				$result1 = $db->query("SELECT asText(SHAPE) as border
                                                                  FROM `MilitaryInstallations`
                                                                  WHERE fullname = '{$line['fullname']}'");
			...
			for($i=0;$i<sizeof($CoordsArray);$i++){
                         ...
                         foreach($CoordsArray[$i] as $c){
                                ...
                                if($line1< sizeof($CoordsArray[$i])-1){
                                       echo ",\n";
								}else{
                                       echo "\n";
                                }
