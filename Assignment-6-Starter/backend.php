...
$sql =  "
    (SELECT AsWKB(eqc.shape) as wkb FROM `state_borders` AS s_b LEFT OUTER JOIN `earth_quakes` AS eqc ON CONTAINS(s_b.shape,eqc.shape) WHERE s_b.state=(SELECT state FROM `state_borders` WHERE CONTAINS (SHAPE, POINT({$_POST['lng']}, {$_POST['lat']}))))
union
(SELECT asWKB(SHAPE) as wkb
		FROM state_borders s_b 
		WHERE CONTAINS(s_b.SHAPE,POINT({$_POST['lng']}, {$_POST['lat']})))
";
